...
app.get('/tbl_usuario/:Id_Usuario', (req, res) => {
    ...
    connection.execute('SELECT * FROM tbl_usuario WHERE Id_Usuario = ?', [Id_Usuario], (err, results) => {
        if (err) {
            res.status(500).send(err);
        } else {
            res.json(results[0]);
...
app.post('/tbl_usuario', (req, res) => {
    ...
    connection.query('INSERT INTO tbl_usuario (Id_Usuario, id_genero, id_area, id_rol, id_sucursal, contraseña, Id_empleado, id_contraseña, nomb_usuario) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [Usuario, genero, area, rol, sucursal, idcontraseña, empleado, contraseña, nomb_usuario], (err, result) => {
        if (err) {
            res.status(500).send(err);
        } else {
